...
@Designate(ocd=LebaraOfferCFStep.Config.class)
public class LebaraOfferCFStep implements WorkflowProcess{
	...
	String contentDamPath = null; 
	private static final String API_PATH = "/api/assets/";
	private static final String DAM_PATH = "/content/dam/";

	@ObjectClassDefinition(name = "CF Domain Name Configuration",	description = "CF Domain Name Configuration")
	public @interface Config {
		@AttributeDefinition(name = "CF Domain Name", description = "CF Domain Name", type = AttributeType.STRING)
		String getCFDomainNamePath() default "http://localhost:4502";
	}

	@Activate
	public void init(Config config) {
		contentDamPath = config.getCFDomainNamePath();
		contentDamPath = contentDamPath + API_PATH;
	}
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap metaDataMap) throws WorkflowException {
		...
		if(destinationFolder != null) {
			contentDamPath = contentDamPath + destinationFolder +"/";
			readEPCAndCreateCF();
		}
	...
	private String getPayloadPath(WorkItem workItem) {
		...
		if(StringUtils.isNotBlank(payload) && payload.startsWith(DAM_PATH)) {
			payload = payload.replaceFirst(DAM_PATH, "");
			return payload;
		}
		return null;
	}

	void readEPCAndCreateCF() {
		try {
	...
	boolean isCFOfferExits(Offer offer){
		URL url;
		try {
			url = new URL (contentDamPath+offer.offerId+".json");
	...
	void writeJsonToCf (Offer offer) {
		...
		URL url;
		try {
			url = new URL (contentDamPath+offer.offerId);
	...
	static String readJsonFromEPC() {
		...
		try {
			...
			url = new URL ("https://sit-omni.lebara.com/sit/epc-configuration/");
			...
			connection.setDoOutput(true);
			connection.setRequestProperty  ("Content-Type", "application/json");
			connection.setRequestProperty  ("Ocp-Apim-Subscription-Key", "4cbbb29f41e346bbb52a02c6bafaffef");
			connection.setRequestProperty  ("Authorization", "Basic " + encoding);
			try(OutputStream os = connection.getOutputStream()) {
				byte[] input = jsonInputString.getBytes("utf-8");
				os.write(input, 0, input.length);			
	...
	static Root getJsonforCF(Offer offer){
		...
		properties.setCqModel("/conf/lebara/settings/dam/cfm/models/epcmodels");
