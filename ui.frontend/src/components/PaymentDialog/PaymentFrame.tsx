...
const { REACT_APP_ADYEN_ENVIRONMENT } = process.env;

// TODO Remove this and use axios with default base url
...
const PaymentFrame: React.FC = () => {
  ...
  const onAdditionalDetails = useCallback(
    (state: any) => {
      fetch(
        `${REACT_APP_HOST_URI}/payments/adyen/payments/details?orderRef=${orderRef.current}`,
        {
          credentials: "include",
          method: "POST",
    ...
    checkout.current = new AdyenCheckout({
      ...
      onSubmit: (state: any, component: any) => {
        if (state.isValid) {
          ...
          fetch(`${REACT_APP_HOST_URI}/payments/adyen/payments`, {
            credentials: "include",
            method: "POST",
            body: JSON.stringify({
              channel: "Web",
